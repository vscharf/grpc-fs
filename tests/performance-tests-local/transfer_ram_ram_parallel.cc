...
public:
                 ...
                 ServerWriter<FileChunk>* writer) override
  {
    ...
    std::cout << "Transferred size: " << transferred_size << std::endl;
    return Status::OK;
  }
private:
...
void ClientGetFile(std::string address = "0.0.0.0:50051") {
  grpc_init();
  ...
  std::cout << "Payload: " << client.payload/1024./1024. << " MB" << std::endl;
  std::cout << "Total:   " << client.total/1024./1024. << " MB" << std::endl;
  std::cout << "Eff:     " << double(client.payload)/client.total << std::endl;
...
int main(int argc, char** argv) {
  ...
  if(server_pid == 0) {
    ...
    auto end = steady_clock::now();
    std::cout << "Total " << duration<double, std::milli>(end - start).count() << " ms" << std::endl;
    if(pid) {
